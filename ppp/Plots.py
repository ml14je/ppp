...
def plot_setup(x_label='', y_label='', x_log=False, y_log=False, bx=10, by=10,
               y_rev=False, scale=1, title='', my_dpi=100):
    """
...
def subplots(rows, cols, scale, x_share=False, y_share=False, my_dpi=100):
    import matplotlib as mpl
...
if __name__=='__main__':
    ...
    y_arr = np.sin(x_arr)
    fig, ax = plot_setup('$x$', '$y$', scale=.5, title='$\sin{(x)}$', dpi=50)
    ax.plot(x_arr, y_arr)
